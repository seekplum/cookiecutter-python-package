...
def checkone(ctx, source):
    ...
    {% if cookiecutter.python_version[0] == "3" %}ctx.run("black --check {source}".format(source=source), echo=True){% endif %}
    ctx.run("flake8 {source}".format(source=source), echo=True)
    {% if cookiecutter.python_version[0] == "3" %}ctx.run("mypy {source}".format(source=source), echo=True){% endif %}


@task(clean)
def check(ctx, job=4):
    """检查代码规范"""
    ctx.run("isort --check-only --diff %s --skip" % package_name, echo=True)
    {% if cookiecutter.python_version[0] == "3" %}ctx.run("black --check %s" % package_name, echo=True){% endif %}
    {% if cookiecutter.use_pylint.lower() in ("y", "yes") %}ctx.run("pylint --rcfile=.pylintrc -j %s --output-format parseable %s" % package_name % job, echo=True)
    ctx.run("pylint --rcfile=.pylintrc -j %s --output-format parseable tests --ignore=test.py "
            "--disable=C0111,W0201,R0201" % job,
            echo=True)
    {% else %}ctx.run("flake8 %s" % package_name, echo=True){% endif %}
    {% if cookiecutter.python_version[0] == "3" %}ctx.run("mypy %s" % package_name, echo=True){% endif %}
...
def coverage(ctx):
    ...
    ctx.run(
        ...
        "coverage report -m --fail-under=100" % package_name,
        encoding="utf-8",
        pty=True,
        echo=True,
    )
...
def format(ctx):
    ...
    {% if cookiecutter.python_version[0] == "3" %}ctx.run("black %s tests" % package_name, echo=True){% endif %}
...
def pylintone(ctx, source, msg_ids="", packages=None, ignore_default=False):
    ...
    inv pylintone --source '{{cookiecutter.project_slug}} *.py' --msg-ids="W1505" --packages="pymongo,celery"
    """
